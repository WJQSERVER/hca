...
func (h *Hca) DecodeFromFile(src, dst string) error {
	...
	if err := h.decodeBuffer(srcFile, dstFile); err != nil {
		// dstFile is closed by defer, os.Remove can proceed.
		os.Remove(dst)
...
func (h *Hca) DecodeFromBytes(data []byte) ([]byte, error) {
	...
	if err := h.decodeBuffer(reader, writer); err != nil {
		return nil, err
	}
...
func (h *Hca) initPools() {
	...
	h.int8Pool.New = func() interface{} {
		return make([]int8, samplesPerBlock)
	}
...
func (h *Hca) decodeBuffer(r io.ReadSeeker, w io.Writer) error {
	if h.Loop < 0 {
		return errors.New("invalid loop count")
	}
	switch h.Mode {
	...
	if _, err := r.Seek(int64(h.dataOffset), 0); err != nil {
...
func (h *Hca) decodeBlocks(r io.ReadSeeker, w io.Writer, address, count uint32) error {
	if _, err := r.Seek(int64(address), 0); err != nil {
	...
	for l := uint32(0); l < count; l++ {
		...
		if err := h.save(saveBlock, w); err != nil {
...
func (h *Hca) save(base []float32, w io.Writer) error {
	switch h.Mode {
	case ModeFloat:
		return binary.Write(w, binary.LittleEndian, base)
	case Mode8Bit:
		buf := h.int8Pool.Get().([]int8)
		defer h.int8Pool.Put(buf)
		mode8BitConvert(buf, base)
		return binary.Write(w, binary.LittleEndian, buf)
	case Mode16Bit:
		buf := h.int16Pool.Get().([]int16)
		defer h.int16Pool.Put(buf)
		mode16BitConvert(buf, base)
		return binary.Write(w, binary.LittleEndian, buf)
	case Mode24Bit:
		buf := h.byte24Pool.Get().([]byte)
		defer h.byte24Pool.Put(buf)
		mode24BitConvert(buf, base)
		_, err := w.Write(buf)
		return err
	case Mode32Bit:
		buf := h.int32Pool.Get().([]int32)
		defer h.int32Pool.Put(buf)
...
func mode8BitConvert(dst []int8, src []float32) {
	for i := range dst {
		dst[i] = int8(int(src[i]*0x7F) + 0x80)
	}
}

func mode16BitConvert(dst []int16, src []float32) {
	for i := range dst {
		dst[i] = int16(src[i] * 0x7FFF)
	}
}
...
func mode24BitConvert(dst []byte, src []float32) {
	for i := range src {
		...
		dst[i*3] = byte((v & 0xFF0000) >> 16)
		dst[i*3+1] = byte((v & 0xFF00) >> 8)
		dst[i*3+2] = byte((v & 0xFF))
	}
}

func mode32BitConvert(dst []int32, src []float32) {
	for i := range dst {
		dst[i] = int32(src[i] * 0x7FFFFFFF)
	}
}
